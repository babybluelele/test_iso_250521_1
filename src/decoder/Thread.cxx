			  ...
			  const InputStream &is) noexcept
{
	...
	if(mime_type != nullptr) FormatNotice(decoder_thread_domain, "probing mine_type %s", mime_type);
...
{
	assert(plugin.stream_decode != nullptr);
	if(suffix != nullptr) FormatNotice(decoder_thread_domain, "probing suffix %s", suffix);
			  ...
			  bool &tried_r)
{
	...
	if (mpd_softwareVolumeStatus_get()) 
	{
		const char *mime_type = is.GetMimeType();
		if(mime_type != nullptr && 
		(strstr(mime_type, "dsf") != nullptr ||
		strstr(mime_type, "dff") != nullptr ||
		strstr(mime_type, "dsd") != nullptr)
		)
		return false;
	}
	if(mpd_softwareVolumeStatus_get() && 
	...
	(0 == strcmp(suffix, "dsf") || 0 == strcmp(suffix, "dff")))
	{
		return false;
	}
	bridge.Reset();
	       ...
	       const DecoderPlugin &plugin)
{
	...
	if (mpd_softwareVolumeStatus_get() && ( 0 == strcmp(suffix, "dsf") || 0 == strcmp(suffix, "dff") ) )
	{
		const struct DecoderPlugin *plugin2;
		plugin2 = decoder_plugin_from_name("ffmpeg");
		bridge.Reset();

		DecoderControl &dc = bridge.dc;

		if (plugin2->file_decode != nullptr) {
			const std::lock_guard<Mutex> protect(dc.mutex);
			return decoder_file_decode(*plugin2, bridge, path_fs);
		} else if (plugin2->stream_decode != nullptr) {
			std::unique_lock<Mutex> lock(dc.mutex);
			return decoder_stream_decode(*plugin2, bridge, input_stream,
							lock);
		} else
			return false;
	}
