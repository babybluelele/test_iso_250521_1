...
{
	locSetAlsa.sndName = sndName;
}

void handle_set_DSDType(std::string & DSDType)
{
	locSetAlsa.DSDType = DSDType;
}

void handle_set_softwareVol(std::string & softwareVol)
{
	locSetAlsa.softwareVol = softwareVol;
	if( true == mpd_softwareVolumeStatus_get() && "off" == softwareVol) exit(0);
	else if( false == mpd_softwareVolumeStatus_get() && "on" == softwareVol) exit(0);
}

void handle_set_allowFormat(std::string & allowFormat)
{
	locSetAlsa.allowFormat = allowFormat;
}