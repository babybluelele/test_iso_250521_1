			       ...
			       const PlaylistPlugin &plugin) noexcept
{
	...
	std::string turi = uri_utf8.c_str();
	if(turi.size() > 4)
	{
		std::string tail = turi.substr(turi.size() - 4, 4);
		if(0 == strcasecmp(".cue", tail.c_str()))
		{
			struct stat st;
			stat(uri_utf8.c_str(), &st);
			if(st.st_size > 1024 * 1024)
			{
				FormatError(update_domain, "scanning playlist '%s'", uri_utf8.c_str());
				FormatError(update_domain, "cue file size over 1M");
				return ;
			}
		}
	}
