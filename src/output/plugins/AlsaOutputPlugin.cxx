...
AlsaOutput::Open(AudioFormat &audio_format)
{
	...
	setAlsa sa;
	handle_get_alsa(sa);
	FormatNotice(alsa_output_domain, "sa.sndName (%s)", sa.sndName.c_str());
	FormatNotice(alsa_output_domain, "sa.dsdType (%s)", sa.DSDType.c_str());
	FormatNotice(alsa_output_domain, "sa.allowFormat (%s)", sa.allowFormat.c_str());
	{
		const std::lock_guard<Mutex> lock(attributes_mutex);
#ifdef ENABLE_DSD
		if(sa.DSDType.size() == 0)
		dop = dop_setting;
		else
		{
			if(sa.DSDType == "dop")
...
#endif
		if(sa.allowFormat.size() != 0)
		{
			std::forward_list<Alsa::AllowedFormat> allowed_formats2;
			allowed_formats2 = Alsa::AllowedFormat::ParseList(sa.allowFormat.c_str());
			const auto &a = BestMatch(allowed_formats2,
				audio_format);
...
dop = a.dop;
	...
	if( 0 == strcmp("hw:NT1,0", GetDevice()) )
	{
		FormatNotice(alsa_output_domain, "open %s", GetDevice());
		err = snd_pcm_open(&pcm, GetDevice(),
			       SND_PCM_STREAM_PLAYBACK, mode);
		if (err < 0)
		{	
			FormatNotice(alsa_output_domain, "reopen %s", GetDevice());
			usleep(500*1000);
			if(err < 0)
			err = snd_pcm_open(&pcm, GetDevice(),
				SND_PCM_STREAM_PLAYBACK, mode);
			if (err < 0)
				throw FormatRuntimeError("again, Failed to open ALSA device \"%s\": %s",
						GetDevice(), snd_strerror(err));
		}
	}
	else if( 0 == strcmp("hw:TT1,0", GetDevice()) )
	{
		FormatNotice(alsa_output_domain, "open %s", GetDevice());
		err = snd_pcm_open(&pcm, GetDevice(),
			       SND_PCM_STREAM_PLAYBACK, mode);
	...
	else if(sa.sndName.size() > 0 )
	{
		FormatNotice(alsa_output_domain, "open %s", sa.sndName.c_str());
		err = snd_pcm_open(&pcm, sa.sndName.c_str(),
			       SND_PCM_STREAM_PLAYBACK, mode);
	}
	else
	{
