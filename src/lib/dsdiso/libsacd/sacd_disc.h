...
class sacd_disc_t:public sacd_reader_t {
 private:
    ...
    uint32_t read_lsn_cnt;
    uint32_t track_play_now;
    uint32_t m_track_duration_s;
 public:
    ...
    int get_framerate();
    float getProgress();
    void get_album_title(char *album_title);
    void get_album_artist(char *album_artist);
    bool is_dst();
    int open(iso_media_t * p_file);
    bool close();
    string set_track(uint32_t track_number, area_id_e area_id = AREA_BOTH, uint32_t offset = 0);
    bool seek_play(unsigned long lsn);
    bool seek_play2(double seconds);
