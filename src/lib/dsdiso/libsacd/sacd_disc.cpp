...
bool iso_media_t::iso_open(const char *path)
{
    try
    {
        iso_file = fopen(path, "r");
        return true;
    }
    catch( ...)
    {
...
bool iso_media_t::iso_seek(int64_t position, int mode)
{
    fseek(iso_file, position, mode);
    return true;
}

size_t iso_media_t::iso_read(void *data, size_t size)
{
    return fread(data, 1, size, iso_file);
}
...
static inline std::string charset_convert(const char* str, size_t insize, uint8_t codepage_index) 
{
...
int sacd_disc_t::open(iso_media_t * p_file)
{
    m_file = p_file;
    ...
    m_sector_bad_reads = 0;
    track_play_now = 0;
    m_file->iso_seek((uint64_t) START_OF_MASTER_TOC * (uint64_t) SACD_LSN_SIZE);

    if(m_file->iso_read(sacdmtoc, 8) == 8)
    {
        if(memcmp(sacdmtoc, "SACDMTOC", 8) == 0)
        {
    ...
    if(!m_file->iso_seek((uint64_t) START_OF_MASTER_TOC * (uint64_t) SACD_PSN_SIZE + 12))
    {
        close();
        return 0;
    }
...
bool sacd_disc_t::close()
{
    if(m_sb.twoch_area_idx != -1)
    {
        ...
        //LELE
        if (m_sb.area[m_sb.twoch_area_idx].area_data) {
    ...
    if(m_sb.mulch_area_idx != -1)
    {
        ...
        if (m_sb.area[m_sb.mulch_area_idx].area_data) {
			free(m_sb.area[m_sb.mulch_area_idx].area_data);
			m_sb.area[m_sb.mulch_area_idx].area_data = nullptr;
		}
        m_sb.mulch_area_idx = -1;
...
void sacd_disc_t::getTrackDetails(uint32_t track_number, area_id_e area_id, TrackDetails * cTrackDetails)
{
    ...
    cTrackDetails->strArtist = m_sb.master_text.album_artist;
    cTrackDetails->strTitle = cAreaTrackText.track_type_title;
    if(m_sb.master_text.album_title.size() != 0)
        cTrackDetails->strAlbum = m_sb.master_text.album_title;
    else
        cTrackDetails->strAlbum = m_sb.master_text.disc_title;
    cTrackDetails->strAlbumArtist = m_sb.master_text.album_artist;
...
string sacd_disc_t::set_track(uint32_t track_number, area_id_e area_id, uint32_t offset)
{
    if(track_number < get_track_count(area_id))
    {
        ...
        m_track_duration_s = area->area_tracklist_time->duration[track_number].minutes * 60 + area->area_tracklist_time->duration[track_number].seconds;
        ...
        m_packet_info_idx = 0;
        m_file->iso_seek((uint64_t) m_track_current_lsn * (uint64_t) m_sector_size);
        ...
        if(area->area_track_text[track_number].track_type_performer.size())
        {
            ...
            sprintf(buf, "(%ich) %.2i. %s - %s.wav", m_channel_count, track_number + 1, area->area_track_text[track_number].track_type_performer.data(),
        ...
        s = stringReplace(s, "/", "-");
        free(buf);
        fprintf(stderr, "start lsn = %d; length = %d; sector size = %d\n", m_track_start_lsn, m_track_length_lsn, m_sector_size);
        read_byte_cnt = 0;
        read_lsn_cnt = 0;
...
bool sacd_disc_t::seek_play2(double seconds)
{
	uint64_t offset = (uint64_t)(m_track_length_lsn * seconds / m_track_duration_s);
	set_track(track_play_now, m_track_area, (uint32_t)(offset / m_sector_size));
	return true;
}

bool sacd_disc_t::seek_play(unsigned long lsn)
{
    int64_t lsn_offset = (int64_t)(m_track_start_lsn + lsn) - (int64_t)m_track_current_lsn;
    int64_t byte_offset = (int64_t)lsn_offset * (int64_t)m_sector_size;
    m_file->iso_seek(byte_offset, SEEK_CUR);
    
    memset(&m_audio_sector, 0, sizeof(m_audio_sector));
    memset(&m_frame, 0, sizeof(m_frame));
    m_packet_info_idx = 0;
    
    m_track_current_lsn = (m_track_start_lsn + lsn);
    read_byte_cnt += byte_offset;
    return true;
}
...
bool sacd_disc_t::read_frame(uint8_t * frame_data, size_t *frame_size, frame_type_e * frame_type)
{
    ...
    while(m_track_current_lsn < m_track_start_lsn + m_track_length_lsn)
    {
        ...
        if(m_packet_info_idx == m_audio_sector.header.packet_info_count)
        {
            ...
            m_buffer_offset = 0;
            m_packet_info_idx = 0;
            size_t read_bytes = m_file->iso_read(m_sector_buffer, m_sector_size);
            if(read_bytes <= 0)
            {
                return false;
            }
    ...
    *frame_type = FRAME_INVALID;
    fprintf(stderr, "read end! lsn cnt = %d; byte cnt = %d\n", read_lsn_cnt, read_byte_cnt);
    return false;
}
...
bool sacd_disc_t::read_blocks_raw(uint32_t lb_start, size_t block_count, uint8_t * data)
{
    switch (m_sector_size)
    {
        case SACD_LSN_SIZE:
        {
            m_file->iso_seek((uint64_t) lb_start * (uint64_t) SACD_LSN_SIZE);

            if(m_file->iso_read(data, block_count * SACD_LSN_SIZE) != block_count * SACD_LSN_SIZE)
            {
                m_sector_bad_reads++;
                return false;
            }
        ...
        case SACD_PSN_SIZE:
        {
            ...
            {
                m_file->iso_seek((uint64_t) (lb_start + i) * (uint64_t) SACD_PSN_SIZE + 12);

                if(m_file->iso_read(data + i * SACD_LSN_SIZE, SACD_LSN_SIZE) != SACD_LSN_SIZE)
                {
                    m_sector_bad_reads++;
                    return false;
                }
    ...
    for(int i = 0; i < MAX_LANGUAGE_COUNT; i++)
    {
        ...
        if(i == 0)
        {
            ...
            if(master_text->album_title_position)
                m_sb.master_text.album_title =
                    charset_convert((char *)master_text + master_text->album_title_position, strlen((char *)master_text + master_text->album_title_position), current_charset);

            if(master_text->album_title_phonetic_position)
                m_sb.master_text.album_title_phonetic =
                    charset_convert((char *)master_text + master_text->album_title_phonetic_position, strlen((char *)master_text + master_text->album_title_phonetic_position), current_charset);

            if(master_text->album_artist_position)
                m_sb.master_text.album_artist =
                    charset_convert((char *)master_text + master_text->album_artist_position, strlen((char *)master_text + master_text->album_artist_position), current_charset);

            if(master_text->album_artist_phonetic_position)
                m_sb.master_text.album_artist_phonetic =
                    charset_convert((char *)master_text + master_text->album_artist_phonetic_position, strlen((char *)master_text + master_text->album_artist_phonetic_position), current_charset);

            if(master_text->album_publisher_position)
                m_sb.master_text.album_publisher =
                    charset_convert((char *)master_text + master_text->album_publisher_position, strlen((char *)master_text + master_text->album_publisher_position), current_charset);

            if(master_text->album_publisher_phonetic_position)
                m_sb.master_text.album_publisher_phonetic =
                    charset_convert((char *)master_text + master_text->album_publisher_phonetic_position, strlen((char *)master_text + master_text->album_publisher_phonetic_position),
                                    current_charset);

            if(master_text->album_copyright_position)
                m_sb.master_text.album_copyright =
                    charset_convert((char *)master_text + master_text->album_copyright_position, strlen((char *)master_text + master_text->album_copyright_position), current_charset);

            if(master_text->album_copyright_phonetic_position)
                m_sb.master_text.album_copyright_phonetic =
                    charset_convert((char *)master_text + master_text->album_copyright_phonetic_position, strlen((char *)master_text + master_text->album_copyright_phonetic_position),
                                    current_charset);
            ...
            if(master_text->disc_title_phonetic_position)
                m_sb.master_text.disc_title_phonetic =
                    charset_convert((char *)master_text + master_text->disc_title_phonetic_position, strlen((char *)master_text + master_text->disc_title_phonetic_position), current_charset);

            if(master_text->disc_artist_position)
                m_sb.master_text.disc_artist =
                    charset_convert((char *)master_text + master_text->disc_artist_position, strlen((char *)master_text + master_text->disc_artist_position), current_charset);

            if(master_text->disc_artist_phonetic_position)
                m_sb.master_text.disc_artist_phonetic =
                    charset_convert((char *)master_text + master_text->disc_artist_phonetic_position, strlen((char *)master_text + master_text->disc_artist_phonetic_position), current_charset);

            if(master_text->disc_publisher_position)
                m_sb.master_text.disc_publisher =
                    charset_convert((char *)master_text + master_text->disc_publisher_position, strlen((char *)master_text + master_text->disc_publisher_position), current_charset);

            if(master_text->disc_publisher_phonetic_position)
                m_sb.master_text.disc_publisher_phonetic =
                    charset_convert((char *)master_text + master_text->disc_publisher_phonetic_position, strlen((char *)master_text + master_text->disc_publisher_phonetic_position), current_charset);

            if(master_text->disc_copyright_position)
                m_sb.master_text.disc_copyright =
                    charset_convert((char *)master_text + master_text->disc_copyright_position, strlen((char *)master_text + master_text->disc_copyright_position), current_charset);

            if(master_text->disc_copyright_phonetic_position)
                m_sb.master_text.disc_copyright_phonetic =
                    charset_convert((char *)master_text + master_text->disc_copyright_phonetic_position, strlen((char *)master_text + master_text->disc_copyright_phonetic_position), current_charset);
        }
...
bool sacd_disc_t::read_area_toc(int area_idx)
{
    ...
    if(area_toc->area_description_phonetic_offset)
        area->description_phonetic =
            charset_convert((char *)area_toc + area_toc->area_description_phonetic_offset, strlen((char *)area_toc + area_toc->area_description_phonetic_offset), current_charset);
...
void sacd_disc_t::get_album_title(char *album_title)
{
    sprintf(album_title, "%s", m_sb.master_text.album_title.data());
}

void sacd_disc_t::get_album_artist(char *album_artist)
{
    sprintf(album_artist, "%s", m_sb.master_text.album_artist.data());
}