...
Player::CheckDecoderStartup(std::unique_lock<Mutex> &lock) noexcept
{
	...
	if (!ForwardDecoderError()) {
		...
		{
			if (handle_mpd_mute_getDacDelay())
			{
				{					
					fprintf(stderr, "LELE: audio in =%s\n",ToString(pc.audio_format).c_str());
					fprintf(stderr, "LELE: audio out=%s\n",ToString(play_audio_format).c_str());
					std::string line = ToString(play_audio_format).c_str();
					bool bret = handle_mpd_mute_parseLine_fist(line);
					if (bret)
					{
						std::cout << "LELE handle_mpd_mute_parseLine_init start 1"  << std::endl;
						{
							const ScopeUnlock unlock(pc.mutex);
							paused = true;
							pc.outputs.Pause();
							pc.outputs.Close();
							pc.state = PlayerState::PAUSE;
							pc.command = PlayerCommand::NONE;
							//idle_add(IDLE_PLAYER);
							//pc.state = PlayerState::STOP;
							//const ScopeUnlock unlock(pc.mutex);
						}
						pc.CommandFinished();									
						handle_mpd_mute_parseLine_do();
						std::cout << "LELE handle_mpd_mute_parseLine_do end 1"  << std::endl;					
					}	
				}
			}
		}
