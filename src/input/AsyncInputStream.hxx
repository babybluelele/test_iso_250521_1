...
public:
	...
	typedef struct ctrlStream ctlstm;
	ctlstm ctlsm;
	Mutex mutex;

	// int dfd;
	// size_t bufferSpaceSize = 0;
	bool checkDecryptoUrl()
	{
		std::string tmpUrl = GetURI();
		if( std::string::npos != tmpUrl.find("http://127.0.0.1:8899/QQPlay?id") ) return true;
		return false;
	}
protected:
	/**
	...
	void AppendBufferStartPos(size_t tsize) noexcept;
	void AppendBufferTotalSize(size_t tsize) noexcept;
	void AppendBufferTotalSizetotal(size_t tsize) noexcept;
...
private:
	...
	void DeferredSeekDown() noexcept;
};
