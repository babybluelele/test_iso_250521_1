...
#include <string.h>
static constexpr Domain BufferingInputStream_domain("bufferingInput");
...
BufferingInputStream::RunThreadLocked(std::unique_lock<Mutex> &lock)
{
	...
	while (!stop) {
		if (want_offset != INVALID_OFFSET) {
			...
			FormatNotice(BufferingInputStream_domain, "*** input->IsEOF(%zu,%zu)", new_offset, buffer.size());
			...
			FormatNotice(BufferingInputStream_domain, "*** input->IsEOF2(%zu, %zu, %d)", new_offset, buffer.size(), i);
			usleep(300*1000);
			i++;
			if(i > 5) break;
	...
	FormatNotice(BufferingInputStream_domain, "RunThreadLocked() done");
}
